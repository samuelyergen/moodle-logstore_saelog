...
class cleanup_task extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('taskcleanup', 'logstore_saelog');
    }
    ...
    public function execute() {
        ...
        $loglifetime = (int)get_config('logstore_saelog', 'loglifetime');
        ...
        while ($min = $DB->get_field_select("logstore_saelog", "MIN(timecreated)", "timecreated < ?", $lifetimep)) {
            ...
            $DB->delete_records_select("logstore_saelog", "timecreated < ?", $params);
            if (time() > $start + 300) {
